...
void Nomis::displayLogo() {
    ...
    for(uint8_t i = 0; i < 6; i++) {
        ...
        sound.tone(baseFreq, toneDuration);
        baseFreq += 512;
    ...
    arduboy.setRGBled(ledOn, 0, ledOn);
...
void Nomis::drawUp() {
    ...
    sound.tone(upFreq, toneDuration);
}
...
void Nomis::drawRight() {
    ...
    sound.tone(rightFreq, toneDuration);
}
...
void Nomis::drawDown() {
    ...
    uint8_t y = 60;
    arduboy.fillTriangle(x, y, x-(triWidth/2), y-triHeight, x+(triWidth/2), y-triHeight, 1);
    sound.tone(downFreq, toneDuration);
}
